...
class VideoAudioExtractor(private val context: Context) {
    ...
    fun extractToPcm(
        ...
        } ?: throw FVoiceException.InvalidInputFile("Cannot open URI: $uri")
        ...
        if (audioTrackIndex == -1 || format == null) {
            extractor.release()
            throw FVoiceException.MediaProbeFailed("No audio track found")
        }
        ...
        FVoiceLogger.i("Extracted audio to PCM: ${outputFile.absolutePath}, size=${outputFile.length()}")
    }
