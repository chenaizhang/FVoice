package com.fvoice.app.core.recording
...
class AudioRecorder(private val context: Context) {
    ...
    fun prepare(): Boolean {
        ...
        return try {
            ...
            FVoiceLogger.e("AudioRecord prepare failed", e)
            false
        }
    }
    ...
    suspend fun startRecording(
        ...
        if (isRecording) {
            FVoiceLogger.w("Already recording")
            return@withContext
        }
        if (audioRecord == null && !prepare()) {
            throw com.fvoice.app.core.exception.FVoiceException.ServiceBindFailed("AudioRecord not initialized")
        }

        val record = audioRecord ?: return@withContext
        isRecording = true
        record.startRecording()
        FVoiceLogger.i("Recording started: ${outputFile.absolutePath}")
    ...
    fun stopRecording() {
        ...
        try {
            audioRecord?.stop()
            FVoiceLogger.i("Recording stopped")
        } catch (e: Exception) {
            FVoiceLogger.e("Error stopping recording", e)
        }
    }
    ...
    fun release() {
        ...
        audioRecord?.release()
        audioRecord = null
        FVoiceLogger.i("AudioRecorder released")
    }
