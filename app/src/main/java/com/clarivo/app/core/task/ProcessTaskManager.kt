...
import com.clarivo.app.core.asr.TranscriptExporter
import com.clarivo.app.core.asr.TranscriptionProcessor
import com.clarivo.app.core.asr.WhisperCppEngine
import com.clarivo.app.core.denoise.AndroidDeepFilterNetEngine
import com.clarivo.app.core.denoise.DenoiseProcessor
import com.clarivo.app.core.denoise.RnNoiseEngine
import com.clarivo.app.core.model.ModelType
import com.clarivo.app.core.model.ModelInfo
import com.clarivo.app.core.media.MediaExportManager
import com.clarivo.app.core.media.MediaProbe
import com.clarivo.app.core.media.OutputNameGenerator
import com.clarivo.app.core.media.VideoAudioExtractor
...
import com.clarivo.app.core.model.TaskSettings
import com.clarivo.app.core.model.TranscriptResult
import com.clarivo.app.core.modelmanager.ModelManager
import com.clarivo.app.core.recording.RecordingSession
import com.clarivo.app.core.vad.SileroVadEngine
import com.clarivo.app.core.vad.VadSegmenter
...
class ProcessTaskManager private constructor(context: Context) {
    ...
    private val videoAudioMuxer = com.clarivo.app.core.media.VideoAudioMuxer(appContext)
    ...
    fun startTask(taskId: String, onStartService: () -> Unit) {
        ...
        scope.launch {
            try {
                ...
                ClarivoLogger.i("Task cancelled: $taskId")
                queue.cancelCurrent()
                ...
                ClarivoLogger.e("Task failed: $taskId", e)
    ...
    private suspend fun processTask(task: ProcessTask, token: ProcessCancellationToken) {
        ...
        val initialLogLines = com.clarivo.app.util.ClarivoLogger.getRecentLogs(1000).size
        ...
        try {
            ...
            } ?: throw com.clarivo.app.core.exception.ClarivoException.MediaProbeFailed("Cannot probe file")
            ...
            if (task.type == ProcessTaskType.DENOISE || task.type == ProcessTaskType.DENOISE_AND_TRANSCRIBE) {
                ...
                val audioInfo = com.clarivo.app.core.model.AudioFormatInfo(
            ...
            if (task.type == ProcessTaskType.TRANSCRIBE || task.type == ProcessTaskType.DENOISE_AND_TRANSCRIBE) {
                ...
                val vadSegments = if (task.settings.useVad && targetSampleRate == 16000) {
                    val vadModel = modelManager.models.value.find { it.type == com.clarivo.app.core.model.ModelType.VAD_SILERO }
                ...
                val asrModel = task.settings.asrModelId
                    ...
                    ?.let { modelId ->
                        modelManager.models.value.find {
                            it.id == modelId && it.status == com.clarivo.app.core.model.ModelStatus.READY
                        }
                    }
                    ?: modelManager.currentAsrModel.value
                        ?.takeIf { it.status == com.clarivo.app.core.model.ModelStatus.READY }
            ...
            try {
                val recentLogs = com.clarivo.app.util.ClarivoLogger.getRecentLogs(500)
    ...
    private fun resolveDenoiseModel(task: ProcessTask): ModelInfo? {
        val readyModels = modelManager.models.value.filter {
            it.status == com.clarivo.app.core.model.ModelStatus.READY
        }
        val requestedModelId = task.settings.denoiseModelId.takeIf { it.isNotBlank() }
        if (requestedModelId != null) {
            return readyModels.find { it.id == requestedModelId }
                ?: throw com.clarivo.app.core.exception.ClarivoException.DenoiseFailed(
