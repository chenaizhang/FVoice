...
import com.clarivo.app.core.jni.DeepFilterNetJni
...
class AndroidDeepFilterNetEngine(
    ...
    override fun isAvailable(): Boolean {
        return try {
            ...
            ClarivoLogger.e("DeepFilterNet native library not loaded", e)
            false
        } catch (e: Exception) {
            ClarivoLogger.e("DeepFilterNet availability check failed", e)
            false
        }
    }
    ...
    override suspend fun initialize(model: ModelInfo, strength: DenoiseStrength): Boolean {
        ...
        if (!file.isFile || file.length() <= 0L) {
            ClarivoLogger.w("DeepFilterNet model file not found: ${model.path}")
            return false
        }
        ...
        return try {
            ...
            if (nativeCtx == 0L) {
                ClarivoLogger.w("DeepFilterNet init returned 0: ${model.name}")
                return false
            }
            jni = instance
            ctx = nativeCtx
            ClarivoLogger.i("DeepFilterNet initialized with official libDF: ${model.name}, ctx=$ctx")
            true
        } catch (e: UnsatisfiedLinkError) {
            ClarivoLogger.e("DeepFilterNet native library not loaded", e)
            false
        } catch (e: Exception) {
            ClarivoLogger.e("DeepFilterNet initialize failed: ${model.name}", e)
            false
        }
    }
    ...
    override suspend fun denoise(
        ...
        val instance = jni ?: throw ClarivoException.DenoiseFailed("DeepFilterNet is not initialized")
        if (ctx == 0L) {
            throw ClarivoException.DenoiseFailed("DeepFilterNet is not initialized")
        }
        if (audioInfo.sampleRate != REQUIRED_SAMPLE_RATE || audioInfo.channels != 1 || audioInfo.bitDepth != 16) {
            throw ClarivoException.DenoiseFailed(
        ...
        try {
            ...
            if (pcmOut.isEmpty()) {
                throw ClarivoException.DenoiseFailed("DeepFilterNet returned empty audio")
            }
            outputPcmFile.writeBytes(pcmOut)
            ClarivoLogger.i("DeepFilterNet done with official libDF: ${outputPcmFile.name}")
        } catch (e: ClarivoException.CancelledByUser) {
            throw e
        } catch (e: Exception) {
            ClarivoLogger.e("DeepFilterNet denoise failed", e)
    ...
    override fun release() {
        if (ctx != 0L && jni != null) {
            try {
                ...
                ClarivoLogger.e("DeepFilterNet release failed", e)
            }
            ctx = 0L
        }
        jni = null
