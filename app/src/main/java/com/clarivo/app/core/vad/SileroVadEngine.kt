...
class SileroVadEngine : VadEngine {
    ...
    override suspend fun initialize(model: ModelInfo): Boolean {
        ...
        if (modelPath.isBlank() || !File(modelPath).exists()) {
            ClarivoLogger.w("SileroVAD model not found at $modelPath")
            return false
        }
        return try {
            env = OrtEnvironment.getEnvironment()
            session = env?.createSession(modelPath, OrtSession.SessionOptions())
            ClarivoLogger.i("SileroVAD ONNX session created: $modelPath")
            true
        } catch (e: Exception) {
            ClarivoLogger.e("SileroVAD init failed", e)
            false
        }
    }
    ...
    override suspend fun detectSegments(
        ...
        if (session == null || env == null) {
            ClarivoLogger.w("SileroVAD session is unavailable")
            return emptyList()
        }

        ClarivoLogger.i("SileroVAD detecting: ${pcmFile.name}")
        return try {
            ...
            ClarivoLogger.e("SileroVAD ONNX inference failed", e)
            emptyList()
        }
    }
