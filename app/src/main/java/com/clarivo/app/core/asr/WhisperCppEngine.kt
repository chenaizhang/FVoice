...
import com.clarivo.app.core.jni.WhisperCppJni
...
class WhisperCppEngine : AsrEngine {
    ...
    override suspend fun initialize(model: ModelInfo): Boolean {
        return try {
            ...
            if (modelPath.isBlank()) {
                ClarivoLogger.w("WhisperCpp model path is blank; transcription will return no segments")
                return false
            }
            jni = WhisperCppJni()
            ctx = jni!!.init(modelPath)
            if (ctx == 0L) {
                ClarivoLogger.w("WhisperCpp init returned 0; transcription will return no segments")
                return false
            }
            ClarivoLogger.i("WhisperCpp initialized: ctx=$ctx")
            true
        } catch (e: UnsatisfiedLinkError) {
            ClarivoLogger.e("Native library not loaded; transcription will return no segments", e)
            false
        } catch (e: Exception) {
            ClarivoLogger.e("WhisperCpp init failed; transcription will return no segments", e)
            false
        }
    }
    ...
    override suspend fun transcribe(
        ...
        if (ctx == 0L || jni == null) {
            throw com.clarivo.app.core.exception.ClarivoException.AsrFailed("WhisperCpp is not initialized")
        }

        ClarivoLogger.i("WhisperCpp transcribing: ${pcmFile.name}")
        return try {
            val samples = audioFileToFloats(pcmFile)
            if (samples.isEmpty()) {
                throw com.clarivo.app.core.exception.ClarivoException.AsrFailed("Input audio has no readable PCM samples")
            }

            cancellationToken?.throwIfCancelled()

            val json = jni!!.transcribe(ctx, samples, samples.size, normalizeLanguage(language))
            ClarivoLogger.i("WhisperCpp raw result: $json")
            if (json.contains("\"error\"")) {
                throw com.clarivo.app.core.exception.ClarivoException.AsrFailed(json)
            }

            parseResult(json, pcmFile)
        } catch (e: com.clarivo.app.core.exception.ClarivoException.CancelledByUser) {
            throw e
        } catch (e: com.clarivo.app.core.exception.ClarivoException.AsrFailed) {
            ClarivoLogger.e("WhisperCpp transcribe failed", e)
            throw e
        } catch (e: Exception) {
            ClarivoLogger.e("WhisperCpp transcribe failed", e)
    ...
    override fun release() {
        if (ctx != 0L && jni != null) {
            try {
                ...
                ClarivoLogger.e("WhisperCpp release failed", e)
            }
            ctx = 0
        }
    }
    ...
    private suspend fun parseResult(json: String, pcmFile: File): List<TranscriptSegment> {
        return try {
            ...
            if (result.segments.isEmpty()) {
                throw com.clarivo.app.core.exception.ClarivoException.AsrFailed("Whisper returned empty segments")
            }
            ...
            ClarivoLogger.e("Failed to parse whisper result; returning empty transcript", e)
            emptyList()
        }
    }
}
