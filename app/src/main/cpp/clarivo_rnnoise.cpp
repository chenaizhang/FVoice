...
#define LOGI(...) __android_log_print(ANDROID_LOG_INFO, "ClarivoRNNoise", __VA_ARGS__)
#define LOGW(...) __android_log_print(ANDROID_LOG_WARN, "ClarivoRNNoise", __VA_ARGS__)
#define LOGE(...) __android_log_print(ANDROID_LOG_ERROR, "ClarivoRNNoise", __VA_ARGS__)
...
Java_com_clarivo_app_core_jni_RnNoiseJni_init(JNIEnv *env, jobject thiz) {
...
Java_com_clarivo_app_core_jni_RnNoiseJni_free(JNIEnv *env, jobject thiz, jlong ctx) {
    if (ctx == 0) return;
...
Java_com_clarivo_app_core_jni_RnNoiseJni_process(
    JNIEnv *env,
    jobject thiz,
    jlong ctx,
    jbyteArray pcm_in,
