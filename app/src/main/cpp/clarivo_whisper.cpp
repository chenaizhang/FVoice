...
#define LOGI(...) __android_log_print(ANDROID_LOG_INFO, "ClarivoWhisper", __VA_ARGS__)
#define LOGE(...) __android_log_print(ANDROID_LOG_ERROR, "ClarivoWhisper", __VA_ARGS__)
...
Java_com_clarivo_app_core_jni_WhisperCppJni_init(JNIEnv *env, jobject thiz, jstring model_path) {
...
Java_com_clarivo_app_core_jni_WhisperCppJni_free(JNIEnv *env, jobject thiz, jlong ctx_ptr) {
    if (ctx_ptr == 0) return;
...
Java_com_clarivo_app_core_jni_WhisperCppJni_transcribe(
    JNIEnv *env,
    jobject thiz,
    jlong ctx_ptr,
    jfloatArray samples,
